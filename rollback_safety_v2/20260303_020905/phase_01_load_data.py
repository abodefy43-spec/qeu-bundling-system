...
def _singularize_english_token(token: str) -> str:
    """Best-effort singularization for normalized english tokens."""
...
def _singularize_english_alias(alias: str) -> str:
    """Return a deterministic singularized variant for an english alias."""
    if not alias or not re.fullmatch(r"[a-z_]+", alias):
        return alias
    parts = [p for p in alias.split("_") if p]
    if not parts:
        return alias
    singular = [_singularize_english_token(p) for p in parts]
    out = "_".join(singular)
    return out if out else alias
...
def build_ingredient_index(recipe_data: dict[str, Any]) -> pd.DataFrame:
    ...
    for key, info in ingredients.items():
        ...
        singular_variants = {_singularize_english_alias(alias) for alias in aliases}
        aliases.update(singular_variants)
        aliases.discard("")
...
def run(force_rebuild: bool = True) -> LoadedData:
    return load_all(force_rebuild=force_rebuild)
