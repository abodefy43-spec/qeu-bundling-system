...
def _resolve_protein_override(norm_name: str) -> str:
    for token, subcategory in PROTEIN_OVERRIDE_SUBCATEGORY.items():
        if re.search(fr"(^| ){re.escape(token)}( |$)", norm_name):
            return subcategory
    return ""
...
def assign_categories(data_dir: Path | None = None) -> pd.DataFrame:
    ...
    for row in products.itertuples(index=False):
        ...
        protein_override = _resolve_protein_override(norm) if not is_non_food else ""
        ...
        if is_non_food:
            ...
            if matched_entries:
                ...
                if protein_override and cat in {"other", "beverages", "spices"}:
                    cat = "protein"
                    sub = protein_override
                    imp = _upgrade_importance(imp, "high")
                    matched_tags.update({"protein", _clean_tag(protein_override)})
            else:
                cat, sub, imp = "other", "", "low"
