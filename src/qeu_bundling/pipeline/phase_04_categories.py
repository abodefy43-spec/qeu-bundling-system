...
from qeu_bundling.pipeline.phase_01_load_data import load_recipe_data
...
def assign_categories(data_dir: Path | None = None) -> pd.DataFrame:
    ...
    require_columns(orders, ["product_id", "product_name", "order_id"], artifact_name="phase_04.filtered_orders")
    require_not_empty(orders, artifact_name="phase_04.filtered_orders")
    ...
    require_columns(
        products,
        ["product_id", "product_name", "category", "subcategory", "category_tags", "frequency_score"],
        artifact_name="phase_04.product_categories",
    )
    require_no_nulls(products, ["product_id", "product_name", "category"], artifact_name="phase_04.product_categories")
    validate_numeric_ranges(
        products,
        rules={"frequency_score": NumericRangeRule(minimum=0.0, maximum=100.0, allow_null=False)},
        artifact_name="phase_04.product_categories",
    )
